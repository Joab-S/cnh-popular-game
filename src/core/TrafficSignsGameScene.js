export default class TrafficSignsGameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .image(width / 2, height / 2, "bg_game_1")
  ...
  setupStartScreen() {
    ...
    const titleText = this.add
      .text(width / 2, height / 2 - 170, "EXAME TEÓRICO", {
    ...
    const targetSignImage = this.add
      .image(width / 2, height / 2 - 100, this.targetSignType.key)
      .setScale(0.08);
    ...
    const instructionText = this.add
      .text(
        width / 2,
        height / 2 - 40,
        `Pressione a BARRA DE ESPAÇO ou clique na placa "${this.targetSignType.name}" quando ela aparecer`,
        {
          fontSize: "18px",
          color: "#000000",
          fontFamily: '"Silkscreen", "Courier New", monospace',
  ...
  setupGameUI() {
    ...
    this.scoreText = this.add
      .text(width / 2, 30, `${this.score}/${this.maxScore}`, {
        fontSize: "32px",
        color: "#ffffff",
        fontFamily: '"Silkscreen", "Courier New", monospace',
        fontWeight: "300",
  ...
  spawnSign() {
    ...
    if (random <= 40 && !this.targetSign) {
      ...
      const nonTargetSigns = this.allSignTypes.filter(
        (sign) => sign.key !== this.targetSignType.key
      );
  ...
  handleSpacePress() {
    if (!this.isGameActive || this.isWaitingToStart || this.spaceKeyCooldown)
      return;
  ...
  endGame(isVictory) {
    ...
    const resultDisplay = this.add
      .text(width / 2, height / 2 - 30, "PARABÉNS!", {
        fontSize: "48px",
        color: "#ffffff",
        fontFamily: '"Silkscreen", "Courier New", monospace',
        fontWeight: "bold",
